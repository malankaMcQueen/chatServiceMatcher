package com.example.matcher.chatService.configuration.kafka;
...
//        configProps.put("reconnect.backoff.max.ms", 100000); // Максимальное время между попытками
    ...
    @Bean
    public ProducerFactory<String, Object> jsonEventProducerFactory() {
        Map<String, Object> configProps = new HashMap<>();
        configProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapAddress);
        configProps.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
    ...
    @Bean
    public KafkaTemplate<String, Object> jsonEventKafkaTemplate() {
        return new KafkaTemplate<>(jsonEventProducerFactory());
    }
